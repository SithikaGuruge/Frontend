...
app.get("/api/employeeInfo/employee", (req, res) => {
  db.query(
    `SELECT e.Employee_ID, p.Pay_Grade,e.Department,b.Branch_Name,e.Gender, e.Name,e.Birthday,e.Marital_status,e.Emergency_contact_Number, es.Status_Type, ej.Job_Title, s.Name as Supervisor_Name FROM employee e natural join employee_pay_grade p natural join employee_status es natural join employee_job_title ej  left join employee s on e.Supervisor_ID = s.Employee_ID  join branch b on e.Branch_ID = b.Branch_ID  where e.Employee_ID = ?`,
    [employeeId],
    (err, rows, fields) => {
      if (err) {
        console.error("Error querying MySQL:", err);
